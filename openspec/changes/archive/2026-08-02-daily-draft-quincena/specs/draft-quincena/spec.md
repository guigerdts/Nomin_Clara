# Delta for Draft Quincena

## Change

Adds the `draft-quincena` capability: incremental daily hour logging with auto-persist. In "Horario Detallado" mode a "Hoy" section lets the worker log one day at a time; each add/update/remove auto-saves to a per-fortnight localStorage key (`nomina-clara-draft-{startDate}`). At fortnight end (or when a stale draft is detected), "Cerrar quincena & save" converts the draft into a `SavedRecord` via the existing save flow. No changes to `scheduleClassifier()` or `calculateBreakdown()`.
